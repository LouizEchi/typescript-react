...
function Events() {
    ...
    const  seedEvent = [
        {'id': '3', 'name': 'First Event','group': 'group Sikat','date': 'November 24, 12:00 PM PST', 'location': 'Within Cebu City', 'restaurants': [{'name': 'Bigbys', 'votes': 3}, {'name': 'mcdo', 'votes': 1}]},
        {'id': '1', 'name': 'The Second Ebento','group': 'Hilaw groip','date': 'November 26, 3:00 PM PST', 'location': 'Within Cebu City', 'restaurants': [{'name': 'casav verd', 'votes': 2}, {'name': 'yello cab', 'votes': 3}]},
        {'id': '2', 'name': 'thrid G','group': 'Okey keyo Grp','date': 'December 5, 7:00 PM PST', 'location': 'Within Cebu City', 'restaurants': [{'name': 'chowking', 'votes': 2}, {'name': 'ucma', 'votes': 4}]},
    ]
            ...
            <div className="columns is-desktop">
                <div className="column">
                    <div className="box create">
                        <p>Event Name</p>
                        <Input field_error={create_field_errors.event_name} label="" type="text" value={event_name} onChange={e => setEventName(e.target.value)} disabled={create_event} />
                        <div className="field">
                            <div className="control">
                                <div className="select is-info is-medium">
                                    <select>
                                        <option>Groups</option>
                                        {user_group_id.map((item, index) => {
                                            return (
                        ...
                        <button className="button is-primary is-info is-rounded" onClick={e => { e.preventDefault(); setCreateEvent(true) }}>Create Event</button>
                    </div>
                    <div className="box my-events">
                        ...
                        {seedEvent.map((item, index) => {
                            return (
                                <div key={index}>
                                    <div onClick={e => setDisplayEvent(item)}><div className="box items">{item.name}</div></div>
                                </div>
                            )
                        })}
                    </div>
                ...
                <div className="column">
                    <div className="box display-event">
                        {display_event ?
                            <div>
                                <div className="name">{display_event.name}</div>
                                <div className="date">{display_event.date}</div>
                                <div className="location">{display_event.location}</div>
                                <hr />
                                <div className="columns is-desktop">
                                    <div className="column restaurant">
                                        <div className="title">Restaurants</div>
                                        {display_event.restaurants.map((item, index) => {
                                            return (
                                                <div key={index}>
                                                    <div>{item.name}</div>
                                                </div>
